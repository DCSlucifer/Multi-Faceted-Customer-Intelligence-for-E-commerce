...
| Task | Result |
|------|--------|
| Cleaning | 1,067,371 → **{{n_clean_rows}}** rows · **{{n_customers_total}}** customers · 41 countries |
| Churn labelling | cutoff `{{churn_cutoff}}`, {{churn_window_days}}-day window · **{{n_customers_labelled}}** labelled · {{churn_rate}} churn |
| Clustering | K-means **k=2** · silhouette **{{clustering_silhouette}}** (std **{{clustering_silhouette_std}}** over 10 seeds) · DBSCAN fails (−0.12) |
| Classification | best **{{classification_best_model}}** test AUC **{{classification_best_auc}}** (CV {{classification_best_cv_auc}}) · MLP **{{classification_mlp_auc}}** |
| Baselines | majority **{{majority_auc}}** · recency-only **{{recency_auc}}** · without-Recency **{{without_recency_auc}}** → model adds real signal |
| Calibration | Brier **{{calibration_brier}}**, reliability curve hugs the diagonal |
| Association | 242 itemsets · **{{association_n_rules}} rules** (lift ≥ 1.5) · top lift **{{top_rule_lift}}** · FP-Growth ~1.4× faster than Apriori |
| **Synthesis** | **{{churn_gap_points}}-pt churn gap** between segments (dormant ~90% vs active ~23%) |
| Anomaly detection | autoencoder flags top **5%** (263 customers) by reconstruction error |
| Business decisions | **{{decision_table_customers}}** customers actioned · **{{decision_retention_targets}}** retention · **{{decision_cross_sell_targets}}** cross-sell · **{{decision_manual_review_targets}}** manual review |
