...
\textbf{Honest evaluation.} To confirm the classifier learns genuine churn signal rather than merely echoing Recency, we benchmark it against honest baselines (Table~\ref{tab:baselines}) and ablate feature groups (Table~\ref{tab:ablation}). The full model (AUC 0.785) clearly beats a majority-class predictor (0.500) and a recency-only model (0.751), and dropping Recency lowers the AUC only to 0.755---so Recency is dominant but \emph{not} sufficient, and the behavioural features add real signal. The predicted probabilities are also well calibrated (Brier 0.189): the reliability curve in Figure~\ref{fig:calib} hugs the diagonal, which matters because the downstream decision layer thresholds directly on these probabilities.
...
\begin{figure}[t]
  ...
  \caption{Reliability curve for the LogisticRegression churn model (10 bins). Predicted probabilities track observed churn frequency closely (Brier 0.189).}
  \label{fig:calib}
\end{figure}
